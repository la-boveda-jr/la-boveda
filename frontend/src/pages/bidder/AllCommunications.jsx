...
import {
    BidderSidebar, BidderHeader, BidderContainer, AccountInactiveBanner, LoadingSpinner,
} from "../../components";
...
const AllCommunications = () => {
    ...
    const fetchCommunications = async () => {
        try {
            setLoading(true);
            const { data } = await axiosInstance.get("/api/v1/communication/bidder/all");
            if (data.success) {
                        ...
                        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
                            <div>
                                ...
                                <p className="text-secondary mt-1">
                                    Chat with the admin about items you've purchased.
                                </p>
                            </div>
                        ...
                        {filtered.length === 0 ? (
                            <div className="bg-white rounded-xl shadow border border-gray-200 p-12 text-center">
                                ...
                                <p className="text-gray-500 mt-1">
                                    Once you win an auction or purchase a product, your conversations
                                    with the admin will appear here.
                                </p>
                            </div>
                        ) : (
                                        ...
                                        <tbody className="bg-white divide-y divide-gray-200">
                                            {filtered.map((comm) => (
                                                <tr key={comm._id} className="hover:bg-gray-50 transition">
                                                    ...
                                                    <td className="px-6 py-4 whitespace-nowrap">
                                                        <Link
                                                            to={`/bidder/communication/${comm?.auction?._id}`}
